...
export const usePermissions = () => {
  ...
  const canManageUsers = () => {
    console.log('canManageUsers check - user role:', user?.user_role);
    return user?.user_role === 'admin' || hasPermission('manage_users');
  };
  ...
  const isAdmin = () => {
    console.log('isAdmin check - user role:', user?.user_role);
