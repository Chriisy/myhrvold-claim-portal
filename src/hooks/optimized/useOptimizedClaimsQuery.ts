...
export const useOptimizedClaimsQuery = (filters: ClaimsQueryFilters = {}) => {
  const {
    searchTerm = '',
    statusFilter = 'Alle',
    categoryFilter = 'Alle',
  ...
  const queryFn = useCallback(async (): Promise<PaginatedResult> => {
    return ErrorService.withRetry(async () => {
      ...
      if (error) {
        console.error('Supabase error in useOptimizedClaimsQuery:', error);
...
export const useOptimizedClaimQuery = (claimId: string) => {
  const queryFn = useCallback(async (): Promise<ClaimWithRelations | null> => {
    if (!claimId) {
      console.warn('No claim ID provided to useOptimizedClaimQuery');
      return null;
    }
    ...
    return ErrorService.withRetry(async () => {
      ...
      if (!isUUID) {
        console.warn('Invalid UUID format provided to useOptimizedClaimQuery:', claimId);
        return null;
      }
      ...
      if (error) {
        console.error('Supabase error in useOptimizedClaimQuery:', error);
